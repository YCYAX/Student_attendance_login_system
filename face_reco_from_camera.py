...
class Face_Recognizer:
    ...
    def show_chinese_name(self):
        ...
        if self.current_frame_face_cnt >= 1:
            for i in range(len(f.iloc[:, :].values.tolist())):
                for a in f.iloc[i:i + 1, 0:5].values.tolist():
                    s = []
                    for b in a:
                        s.append(str(b))
                    self.face_name_known_list[i] = ' '.join(s)
            # 修改录入的人脸姓名 / Modify names in face_name_known_list to chinese nameq
            #self.face_name_known_list[0] = '陈雅文'.encode('utf-8').decode()
            #self.face_name_known_list[1] = '田义杨'.encode('utf-8').decode()
            #self.face_name_known_list[2] = '张丹萌'.encode('utf-8').decode()
                ...
                if kk == ord('q'):
                    ...
                    if len(faces) != 0:
                        ...
                        for k in range(len(faces)):
                            ...
                            if min(current_frame_e_distance_list) < 0.3:
    ...
    def run(self):
        ...
        cap.set(1, 640)# Get video stream from camera
        cap.set(2, 480)                         # 640x480
        self.process(cap)
